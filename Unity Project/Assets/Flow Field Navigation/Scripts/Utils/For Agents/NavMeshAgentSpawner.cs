...
public class NavMeshAgentSpawner : MonoBehaviour
{
    ...
    public void Spawn()
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            Vector3 world = center + rotation * new Vector3(p.x, 0f, p.y);
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        Vector3 prev = transform.position + rotation * new Vector3(radius, 0f, 0f);

        for (int i = 1; i <= segments; i++)
        {
            float angle = i * Mathf.PI * 2f / segments;

            Vector3 next = transform.position + rotation * new Vector3(
        ...
        Gizmos.DrawRay(transform.position, transform.up * radius * 0.5f);
    }
}